...
(function (window, document) {
    ...
    QsmartDetailpanel.prototype._render = function () {
        ...
        var header = "<header style='z-index:991'><div class='qsm-mui-header-v'><h1>" + me.title +
                     "</h1><span id='" + me.backid + "' class='qsm-mui-menu back first'>&nbsp;</span>" +
                     "<span class='qsm-mui-menu ok' id='" + me.okid + "'></span>";
        if (me.options.moresetting) {
            header = header + "<span class='qsm-mui-menu more' id='" + me.moreid + "'></span>";
        }
        ...
        var footer = "<div class='footer' id='" + me.footerid + "' style='z-index:992;'>";
        if (me.options.buttons != undefined && me.options.buttons != '') {
            footer = footer + me.options.buttons;
        }
        footer = footer + "</div>";
        $("body").append(div);
    ...
    QsmartDetailpanel.prototype._resize = function () {
        ...
        var itemlength = liheight + topadd;
    ...
    QsmartDetailpanel.prototype._bind = function () {
        ...
        $('#' + me.okid).on(me.options.action, function (e) {
            e.stopPropagation();
            $('#' + me.contentid + " .qsm-panel-htm").trigger('beforesure', [me]);
            me._hide();
        });
        $('#' + me.okid).on(me.options.mdownaction, function (e) {
            $(this).addClass('down');
        });
        $('#' + me.okid).on(me.options.mupaction, function (e) {
