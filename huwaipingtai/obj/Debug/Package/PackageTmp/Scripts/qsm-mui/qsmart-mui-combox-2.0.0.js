...
(function (window, document) {
    ...
    QsmartCombox.prototype._resize = function () {
        ...
        if (me.scoller == undefined) {
            setTimeout(function () {
                me.scoller = new IScroll('#' + me.wrapperid, { mouseWheel: true, click: true, autorefresh: me._autorefresh, hostobject: me });
            }, 1);
        }
        else {
            me.scoller.refresh();
    ...
    QsmartCombox.prototype._autorefresh = function (iscroll) {
        var me = iscroll.options.hostobject;
        if (me == undefined) return;
        var options = me.options;
        if (options.pagation == false) return;
        var curcount = options.pagesize * (options.pageindex - 1);
        if (curcount >= me.total) return;
        var di = iscroll.maxScrollY - iscroll.y;
        if (di > 80 && $('.qsm-mui-autorefresh')[0] == undefined) {
            iscroll.disable();
            me._appendrotatebar();
            ...
            setTimeout(function () {
                ...
                iscroll.maxScrollY = iscroll.wrapper.clientHeight - iscroll.scroller.offsetHeight;
                y = iscroll.maxScrollY;
                iscroll.enable();
            }, 100);
        }
    };

    QsmartCombox.prototype._appendrotatebar = function () {
        var me = this;
        ...
        $('#' + me.contentid + ' ul').append("<li class='qsm-mui-autorefresh' id = '" + me.morebuttoncontent + "'>" + rotatebarhtml + "</li>");
    };
    QsmartCombox.prototype._removerotatebar = function () {
        var me = this;
        $('#' + me.morebuttoncontent).remove();
    };
